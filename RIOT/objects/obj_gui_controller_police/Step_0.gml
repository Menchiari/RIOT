...
if (device_mouse_check_button_released(0,mb_any) && !(ai==true && ai_faction==1))
^^ (ai==true && ai_faction==1)
{
///////////////////////////////POLICE\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
	if (point_distance(mouse_x,mouse_y,b1_police_x,b1_police_y)<range && b1_police_locked==false)
	&& !(ai==true && ai_faction==1)
	|| (ai==true && ai_choice==1 && b1_police_locked==false)
	{
		instance_create(b1_police_x,b1_police_y,obj_gui_tap);
		if instance_exists(obj_camera_global) {obj_camera_global.journalists_police+=1;}
		ai_choice=0;
		//Journalists
	...
	if (point_distance(mouse_x,mouse_y,b2_police_x,b2_police_y)<range && b2_police_locked==false)
	&& !(ai==true && ai_faction==1)
	|| (ai==true && ai_choice==2 && b2_police_locked==false)
	{
		instance_create(b2_police_x,b2_police_y,obj_gui_tap);
		if instance_exists(obj_camera_global) {obj_camera_global.slow_police+=1;}
		ai_choice=0;
		//Slow
	...
	if (point_distance(mouse_x,mouse_y,b3_police_x,b3_police_y)<range && b3_police_locked==false)
	&& !(ai==true && ai_faction==1)
	|| (ai==true && ai_choice==3 && b3_police_locked==false)
	{
		instance_create(b3_police_x,b3_police_y,obj_gui_tap);
		if instance_exists(obj_camera_global) {obj_camera_global.fast_police+=1;}
		ai_choice=0;
		//Fast
	...
	if (point_distance(mouse_x,mouse_y,b4_police_x,b4_police_y)<range && b4_police_locked==false)
	&& !(ai==true && ai_faction==1)
	|| (ai==true && ai_choice==4 && b4_police_locked==false)
	{
		instance_create(b4_police_x,b4_police_y,obj_gui_tap);
		if instance_exists(obj_camera_global) {obj_camera_global.ranged_police+=1;}
		ai_choice=0;
		//Ranged
